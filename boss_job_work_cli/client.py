"""HTTP client for boss-job-work FastAPI backend."""

import os
import httpx

BASE_URL = os.environ.get("BOSS_JOB_WORK_API") or os.environ.get("LAKEJOB_API", "http://127.0.0.1:8010")


def _post(path: str, json=None, timeout=120):
    try:
        resp = httpx.post(f"{BASE_URL}{path}", json=json, timeout=timeout, trust_env=False)
    except httpx.ConnectError:
        resp = httpx.Response(503, text="Cannot connect to boss-job-work server. Run `boss-job-work server --start` first.")
    return resp


def _get(path: str, timeout=30):
    try:
        resp = httpx.get(f"{BASE_URL}{path}", timeout=timeout, trust_env=False)
    except httpx.ConnectError:
        resp = httpx.Response(503, text="Cannot connect to boss-job-work server. Run `boss-job-work server --start` first.")
    return resp
...
def remove_shortlist(sid: int):
    resp = httpx.delete(f"{BASE_URL}/api/shortlists/{sid}", timeout=30, trust_env=False)
    return resp

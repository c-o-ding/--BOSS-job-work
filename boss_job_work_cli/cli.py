"""boss-job-work CLI — BOSS直聘岗位雷达命令行工具."""
...
@click.group()
def main():
    """boss-job-work — BOSS直聘岗位雷达 v0.1.0
...
@click.option("--salary-min", type=float, default=None, help="最低薪资下限，单位K，如 8")
@click.option("--exclude", default=None, help="屏蔽关键词，逗号分隔，如 专家,经理")
@click.option("--count", type=int, default=600, help="返回条数上限")
def search_cmd(keyword, city, welfare, salary_min, exclude, count):
    """搜索BOSS直聘岗位。"""
    resp = client.search(keyword, city, count, welfare=welfare, salary_min=salary_min, exclude_keywords=exclude)

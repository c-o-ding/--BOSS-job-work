...
class BossScraper:
    def __init__(self, headless=False):
        ...
        self.page = None
        self._closing_extra_pages = False

    def _cleanup_profile_lock_files(self):
    ...
    def _kill_profile_firefox_processes(self, wait_seconds=5):
        if os.name != "nt":
            return
...
Start-Sleep -Milliseconds {max(500, int(wait_seconds * 1000))}
"""
        try:
            subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=max(10, wait_seconds + 8),
            )
        except Exception:
            pass

    def _prepare_profile_launch(self):
        PROFILE_DIR.mkdir(parents=True, exist_ok=True)
        self._kill_profile_firefox_processes(wait_seconds=1)
        self._cleanup_profile_lock_files()

    @staticmethod
    def _should_retry_launch(exc: Exception) -> bool:
        text = str(exc).lower()
        return any(
            key in text
            for key in (
    ...
    def start(self):
        ...
        for attempt in range(2):
            try:
                ...
                self.page = self._pick_primary_page()
                self._close_extra_pages()
                self._bind_single_tab_guard()
                self.page.set_default_timeout(30000)
                return
            except Exception as exc:
                last_error = exc
                self.close()
                if attempt == 0 and self._should_retry_launch(exc):
                    self._prepare_profile_launch()
                    time.sleep(1)
                    continue
                raise last_error

    def _pick_primary_page(self):
        pages = [p for p in list(self._ctx.pages or []) if self._page_alive(p)]
        if not pages:
        ...
        def score(p):
            ...
            if "/web/geek/job" in url:
                return 70
            if "zhipin.com" in url and "/gongsi/" not in url:
                return 50
    ...
    def _page_alive(self, page=None) -> bool:
    ...
    def _ensure_active_page(self):
        """保证 self.page 指向一个未关闭页面，并顺手清理多余标签。"""
        if not self._ctx:
            raise RuntimeError("浏览器上下文未启动")
        if not self._page_alive(self.page):
            self.page = self._pick_primary_page()
            try:
                self.page.set_default_timeout(30000)
            except Exception:
                pass
        self._close_extra_pages(keep=self.page)
        return self.page

    def _bind_single_tab_guard(self):
        if not self._ctx:
            return
        ...
        def on_page(new_page):
            try:
                time.sleep(0.8)
                url = new_page.url or ""
                if any(path in url for path in ("/web/geek/chat", "/job_detail/", "/web/geek/job")):
                    old_page = self.page
                    self.page = new_page
                    self.page.set_default_timeout(30000)
                    self._close_extra_pages(keep=self.page)
                    try:
                        if old_page and old_page is not self.page and not old_page.is_closed():
                            old_page.close()
                    except Exception:
                        pass
                    return
    ...
    def _close_extra_pages(self, keep=None):
        if not self._ctx or self._closing_extra_pages:
            return
        self._closing_extra_pages = True
        try:
            keep = keep or self.page or self._pick_primary_page()
            if not self._page_alive(keep):
                keep = self._pick_primary_page()
            for p in list(self._ctx.pages or []):
                if p is keep:
                    continue
                try:
                    if not p.is_closed():
    ...
    def close(self):
        ctx, br, pw = self._ctx, self._br, self._pw
        self.page = None
        self._ctx = None
        self._br = None
        self._pw = None
        ...
        killer = None
        if ctx or br or pw:
            killer = threading.Timer(3.0, self._kill_profile_firefox_processes, kwargs={"wait_seconds": 1})
            killer.daemon = True
            killer.start()
    ...
    def search(self, keyword, city_code="100010000", limit=600):
        """搜索关键词，返回岗位列表"""
        self._ensure_active_page()
        url = "https://www.zhipin.com/web/geek/jobs?query=%s&city=%s&sortType=1" % (
            quote_plus(keyword),
            city_code,
        )
        ...
        api_jobs = self._fetch_search_jobs_api(keyword, city_code, limit=limit)
        if api_jobs:
            return self._sort_jobs_by_freshness(api_jobs)

        self._scroll_all()

        dom_jobs = self._extract_job_cards()
        if dom_jobs:
            return self._sort_jobs_by_freshness(dom_jobs)
        ...
        if links:
            ...
            for j in jobs:
                if not j["url"] and j["title"][:12] in lm:
                    j["url"] = lm[j["title"][:12]]
        return self._sort_jobs_by_freshness(jobs)

    def _fetch_search_jobs_api(self, keyword, city_code="100010000", limit=600, page_size=30):
        """Fetch paged BOSS search results. sortType=1 asks BOSS for newest-first ordering."""
        jobs = []
        seen = set()
        max_pages = max(1, min(20, (int(limit or page_size) + page_size - 1) // page_size))
            ...
            try:
                data = self.page.evaluate(
                    """async (params) => {
                        ...
                        const resp = await fetch('/wapi/zpgeek/search/joblist.json?' + qs.toString(), {
                            credentials: 'include',
                ...
                print(f"  [搜索API] 第{page}页失败: {e}")
                break

            zp = ((data or {}).get("zpData") or {})
            rows = zp.get("jobList") or zp.get("geekJobList") or []
            if not rows:
                break
            before = len(jobs)
            for row in rows:
                job = self._job_from_api_row(row)
                url = job.get("url", "")
                if not url or url in seen:
                    continue
                seen.add(url)
                job["_source_order"] = len(jobs)
                jobs.append(job)
                if limit and len(jobs) >= limit:
                    return jobs
            if len(jobs) == before:
                break
            if zp.get("hasMore") is False or zp.get("hasNext") is False:
                break
            pause(0.5, 1.0)
        return jobs

    def _job_from_api_row(self, row):
        encrypt_id = row.get("encryptJobId") or row.get("jobId") or ""
        security_id = row.get("securityId") or ""
        url = row.get("jobUrl") or row.get("url") or ""
        if not url and encrypt_id:
            url = "https://www.zhipin.com/job_detail/%s.html" % encrypt_id
            if security_id:
                url += "?securityId=%s" % quote_plus(security_id)
        publish_time = (
            row.get("jobFreshness")
            or row.get("refreshTime")
            or row.get("publishTime")
            or row.get("publishTimeDesc")
            or row.get("updateTimeDesc")
            or row.get("lastModifyTime")
            or ""
        )
        city = row.get("cityName") or row.get("areaDistrict") or row.get("businessDistrict") or ""
        if row.get("areaDistrict") and row.get("businessDistrict"):
            city = "%s·%s" % (row.get("areaDistrict"), row.get("businessDistrict"))
        skills = row.get("skills") or row.get("skillsDesc") or ""
        if isinstance(skills, list):
            skills = " ".join(str(x) for x in skills)
        return {
            "title": row.get("jobName") or row.get("title") or "",
            "salary": decode_salary(row.get("salaryDesc") or row.get("salary") or ""),
            "company": row.get("brandName") or row.get("companyName") or "",
            "experience": row.get("jobExperience") or row.get("experienceName") or "",
            "education": row.get("jobDegree") or row.get("degreeName") or "",
            "city": city,
            "url": url,
            "description": str(skills),
            "hr_name": row.get("bossName") or row.get("hrName") or "",
            "hr_title": row.get("bossTitle") or row.get("hrTitle") or "",
            "welfareList": row.get("welfareList") or row.get("benefitList") or [],
            "publish_time": str(publish_time),
        }

    def _freshness_rank(self, text):
        text = str(text or "").strip()
        if not text:
            return 9999
        if any(k in text for k in ("刚刚", "今天", "今日", "新发布")):
            return 0
        if "昨天" in text:
            return 1
        m = re.search(r"(\d+)\s*(分钟|小时|天|周|个月|月)", text)
        if not m:
            return 9999
        n = int(m.group(1))
        unit = m.group(2)
    ...
    def _sort_jobs_by_freshness(self, jobs):
        return sorted(
            jobs,
            key=lambda j: (
                self._freshness_rank(j.get("publish_time") or j.get("freshness") or j.get("update_time")),
                j.get("_source_order", 0),
            ),
        )

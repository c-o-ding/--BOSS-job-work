...
```bash
boss-job-work server --start --port 8010    # 启动后台服务
boss-job-work doctor                         # 环境诊断
boss-job-work schema                         # 获取工具清单
boss-job-work status                         # 检查登录态
```

完成标准：
- `boss-job-work doctor` 返回 `ok=true`
- `boss-job-work schema` 返回 12 个可用命令
- `boss-job-work status` 返回 `browser_running: true`
...
```bash
# 1. 搜索岗位
boss-job-work search "AI Agent" --city 广州 --welfare "双休,五险一金"

# 2. 查看岗位列表
boss-job-work jobs --status pending --limit 20

# 3. AI 分析某个岗位匹配度
boss-job-work analyze <job_url> --title "AI开发工程师"

# 4. 收藏感兴趣的岗位
boss-job-work shortlist add --job-url <job_url> --title "AI开发"
...
| `false` + HTTP 503 | 服务未启动 | 提示用户 `boss-job-work server --start` |
...
def boss_job_work_cmd(*args):
    result = subprocess.run(["boss-job-work"] + list(args), capture_output=True, text=True)
    return json.loads(result.stdout)

# 搜索
r = boss_job_work_cmd("search", "AI Agent", "--city", "北京")
if r["ok"]:
    jobs = r["data"]
    print(f"找到 {r['total']} 个岗位")

# 诊断
r = boss_job_work_cmd("doctor")
...
function bossJobWork(...args: string[]) {
  const stdout = execSync(`boss-job-work ${args.join(' ')}`).toString();
  return JSON.parse(stdout);
}

const result = bossJobWork('search', 'Golang', '--city', '广州');
...
```bash
result=$(boss-job-work search "Golang" --city 北京)
...
1. 先运行 `boss-job-work doctor` 检查环境
2. 若 `browser_running: false`，提示用户在浏览器登录
3. 运行 `boss-job-work schema` 获取最新工具列表（不要硬编码命令）
4. 根据用户意图调用对应命令

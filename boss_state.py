...
def init_db():
    ...
    db.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS conversations (
            ...
            last_auto_reply_signature TEXT,
            last_auto_reply_text TEXT,
    ...
    try:
        ...
        pass
    try:
        db.execute("ALTER TABLE conversations ADD COLUMN last_auto_reply_signature TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        db.execute("ALTER TABLE conversations ADD COLUMN last_auto_reply_text TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        db.execute("ALTER TABLE conversations ADD COLUMN last_auto_reply_at TIMESTAMP")
    except sqlite3.OperationalError:
        pass
    for ddl in (
        "ALTER TABLE applications ADD COLUMN dedup_key TEXT",
        "ALTER TABLE applications ADD COLUMN fit_score INTEGER DEFAULT 0",
        "ALTER TABLE applications ADD COLUMN fit_level TEXT",
        "ALTER TABLE applications ADD COLUMN fit_recommendation TEXT",
        "ALTER TABLE applications ADD COLUMN fit_reason TEXT",
        "ALTER TABLE applications ADD COLUMN fit_detail TEXT",
    ):
        try:
            db.execute(ddl)
        except sqlite3.OperationalError:
            pass
    db.execute("CREATE INDEX IF NOT EXISTS idx_applications_dedup_key ON applications(dedup_key)")
    # 候选池表
    db.executescript("""
    ...
    cur = db.execute(
        """INSERT OR IGNORE INTO applications
           (job_title, company, salary, job_url, city, experience, education, hr_name, hr_title, description,
            status, dedup_key, fit_score, fit_level, fit_recommendation, fit_reason, fit_detail)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            ...
            job.get("hr_title", ""),
            job.get("description", ""),
            job.get("status") or ("pending" if (job.get("url") or job.get("job_url")) else "missing_url"),
            job.get("dedup_key", ""),
            int(job.get("fit_score") or 0),
            job.get("fit_level", ""),
            job.get("fit_recommendation", ""),
            job.get("fit_reason", ""),
            job.get("fit_detail", ""),
        ),
    )
...
def get_application_by_dedup_key(dedup_key: str) -> Optional[dict]:
    if not dedup_key:
        return None
    return _row_to_dict(
        get_db().execute("SELECT * FROM applications WHERE dedup_key=? ORDER BY id ASC LIMIT 1", (dedup_key,)).fetchone()
    )
...
def update_application_from_job(app_id: int, job: dict) -> Optional[dict]:
    ...
    fields = {
        ...
        "fit_score": str(job.get("fit_score", "")) if job.get("fit_score") is not None else "",
        "fit_level": job.get("fit_level", ""),
        "fit_recommendation": job.get("fit_recommendation", ""),
        "fit_reason": job.get("fit_reason", ""),
        "fit_detail": job.get("fit_detail", ""),
    }
    params = []
    assignments = []
...
def update_application_intelligence(app_id: int, job: dict, update_status: bool = True) -> Optional[dict]:
    """更新评分/去重字段；只在未投递状态下自动调整 pending/skipped。"""
    db = get_db()
    current = get_application(app_id) or {}
    status = current.get("status") or "pending"
    next_status = status
    if update_status and status in ("pending", "failed", "skipped"):
        recommendation = job.get("fit_recommendation") or ""
        next_status = "skipped" if recommendation == "skip" else "pending"
    db.execute(
        """UPDATE applications SET dedup_key=?, fit_score=?, fit_level=?, fit_recommendation=?,
           fit_reason=?, fit_detail=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?""",
        (
            job.get("dedup_key", ""),
            int(job.get("fit_score") or 0),
            job.get("fit_level", ""),
...
def list_applications(status: Optional[str] = None, limit: int = 50) -> List[dict]:
    db = get_db()
    if status in ("submitted", "records"):
        rows = db.execute(
            "SELECT * FROM applications WHERE status IN ('applied', 'replied') ORDER BY updated_at DESC LIMIT ?",
            (limit,),
        ).fetchall()
    elif status:
        rows = db.execute(
...
def clear_pending_applications() -> int:
    db = get_db()
    cur = db.execute("DELETE FROM applications WHERE status='pending'")
    db.commit()
...
def mark_auto_reply_signature(conv_id: int, signature: str, reply_text: Optional[str] = None):
    if reply_text is None:
        get_db().execute(
            """UPDATE conversations SET last_auto_reply_signature=?,
               last_auto_reply_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE id=?""",
            (signature, conv_id),
        )
    else:
        get_db().execute(
            """UPDATE conversations SET last_auto_reply_signature=?,
               last_auto_reply_text=?, last_auto_reply_at=CURRENT_TIMESTAMP,
               updated_at=CURRENT_TIMESTAMP WHERE id=?""",
            (signature, reply_text[:300], conv_id),
        )
    get_db().commit()
